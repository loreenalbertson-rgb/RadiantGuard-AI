...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.caption(
            "Do not upload files containing patient-identifying information."
        )
...
def render_study(study: ImageStudy) -> None:
    ...
    if study.metadata:
        with st.expander(
            "View non-identifying image metadata"
        ):
            meta_cols = st.columns(2)

            for index, (key, value) in enumerate(
                study.metadata.items()
            ):
                meta_cols[index % 2].text(
                    f"{key}: {value}"
                )
    ...
    st.markdown(
        ...
        <div class="model-placeholder">
            ...
            <p>
                The next model phase will add a validated public chest
                X-ray research model, calibrated confidence, and
                explainability overlays.

                Until then, this workspace will never generate a
                medical finding from an upload.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_analyze_tab() -> None:
    ...
    with intro:
        ...
        st.write(
            "Review how a medical image is represented, inspect "
            "technical pixel characteristics, and prepare it for "
            "future explainability research."
        )
    ...
    with checklist:
        st.markdown(
            ...
            <div class="safety-strip">
                ...
                <span>
                    Use only de-identified educational or research images.
                </span>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    consent = st.checkbox(
        "I understand this is a non-diagnostic prototype and I will "
        "not upload identifiable patient information.",
        key="upload_consent",
    )
    ...
    uploaded_file = st.file_uploader(
        ...
        help=(
            "Supported in this foundation build: "
            "PNG, JPG, JPEG, DCM, and DICOM."
        ),
    )
    ...
    try:
        ...
        st.error(
            "RadiantGuard could not safely display this file."
        )
...
def render_safety_lab_tab() -> None:
    ...
    st.write(
        "Paste an AI-generated imaging statement to review its wording "
        "for common safety concerns. This first rules-based auditor "
        "evaluates language—not the medical accuracy of the report."
    )
    ...
    report_text = st.text_area(
        ...
        height=190,
        placeholder=(
            "Example: This image definitely confirms pneumonia "
    ...
    if not report_text.strip():
        st.markdown(
            ...
            <div class="empty-state compact">
                ...
                <p>
                    The auditor will flag overconfidence, missing
                    human-review language, and unsupported
                    replacement claims.
                </p>
            </div>
            """,
    ...
    status_col.caption(
        "A language-quality indicator only; "
        "not a clinical accuracy score."
    )
    ...
    if audit.flags:
        ...
        st.success(
            "No high-priority wording flags were found "
    ...
    if audit.positive_signals:
        ...
        st.info(
            "Consider adding explicit uncertainty and qualified "
            "physician or radiologist review language."
        )

    st.markdown("### Safer prototype wording")

    st.code(
        audit.suggested_footer,
        language=None,
    )
...
def render_about_tab() -> None:
    st.markdown(
        "## Building transparent imaging AI"
    )

    st.write(
        "RadiantGuard AI is an educational research sandbox focused "
        "on explainability, uncertainty, human oversight, and the "
        "evaluation of AI-generated medical-imaging outputs."
    )
    ...
    with c2:
        ...
        st.write(
            "Public chest X-ray model, reproducible preprocessing, "
            "calibrated output, and model cards."
        )
    ...
    with c3:
        ...
        st.write(
            "Heatmaps, uncertainty, report comparison, model "
            "disagreement, and AI trust evaluation."
        )
    ...
    st.write(
        "Students, healthcare AI evaluators, developers, researchers, "
        "and clinicians exploring responsible medical-imaging AI—not "
